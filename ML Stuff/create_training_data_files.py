...
def rm_repeat_boss(df):
    first_kill = df.query('kill == 1.0')
    if len(first_kill) > 0:
        first_time = first_kill.loc[first_kill['fight_start_time'].idxmin()]['fight_start_time']
        df = df.query(f'fight_start_time < {first_time+10}')

    df = df.sort_values(by = ['fight_start_time']).reset_index(drop = True)
    ...
    for index, row in df[1:].iterrows():
        if abs(row['fight_start_time'] - last_start)/1000 > 30:
            if row['boss_perc'] != last_perc:#row['boss_perc'] > 0 and
                idx_s.append(index) 
...
for boss in boss_names:
    ...
    with open('pull_list_'+str(boss.replace(' ','_'))+'.csv', 'w') as f:
        ...
        csv_out.writerow(['pulls','ilvl','kills'])
